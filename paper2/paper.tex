...
\end{prop}
\begin{proof}
By Lemma \ref{cocycle}, $\rho_{n,k}$ is a cocycle. The homomorphism $\rho_{n,k}$ is not a coboundary by the following argument: Assume there exists $g\in C^{l-1}$ with $\rho_{n,k}=\partial^*g$. If we restrict $\rho_{n,k}$ to $\Delta_{n,k}$, this implies $\delta_{\sigma_{n,k}}=\partial^*\widetilde g$, where $\widetilde g$ is the restriction of $g$ to $\Delta_{n,k}$. It remains to show that $\rho_{n,k}$ is a generator. Assume there exists $f\in\hom(C_l,\Z)$ such that $[\rho_{n,k}]=k[f]$ with $k>1$. Again we restrict to $\Delta_{n,k}$ and obtain $[\delta_{\sigma_{n,k}}]=k[\widetilde f]$. By the discussion above this is a contradiction.
\end{proof}
